...
class Edrone():
    ...
    # Note: The imu publishes various kinds of data viz angular velocity, linear acceleration, magnetometer reading (if present),
    ...
    # We need to convert the quaternion format to Euler angles format to understand the orientation of the eDrone in an easy manner.
    ...
    def drone_command_callback(self, msg):
        ...
        # Move up 7 meters
        if self.drone_orientation_euler[2] * (180/pi) < 70:  # Assuming yaw angle is in degrees
            self.setpoint_cmd[3] = 1700  # Increase throttle to move up
        # Move down 4 meters
        elif self.drone_orientation_euler[2] * (180/pi) > 10:  # Assuming yaw angle is in degrees
            self.setpoint_cmd[3] = 1300  # Decrease throttle to move down
    ...
    def pid(self):
        ...
        #   5. Calculate the pid output required for each axis. For eg: calculate self.out_roll, self.out_pitch, etc.
        ...
        #   7. Don't run the PID continuously. Run the PID only at a sample time. self.sampletime defined above is for this purpose. THIS IS VERY IMPORTANT.
        ...
        #   9. Add error_sum to use for the integral component
...
if __name__ == '__main__':
    ...
    e_drone = Edrone()
    r = rospy.Rate(1/e_drone.sample_time)  # Specify rate in Hz based upon your desired PID sampling time, i.e. if the desired sample time is 33ms specify rate as 30Hz
